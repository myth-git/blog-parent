...
import com.sise.common.constant.CacheKey;
import com.sise.gateway.api.ResourceApi;
import com.sise.gateway.utils.JwtUserInfo;

import javax.annotation.Resource;
import javax.servlet.http.HttpServletRequest;
import java.util.List;

import com.sise.gateway.utils.RedisUtil;

import java.util.Objects;

import static com.sise.common.constant.RedisConst.TOKEN_ALLOW_LIST;
import static com.sise.common.constant.RedisConst.USER_RESOURCE_KEY;
import static com.sise.common.enums.StatusCodeEnum.*;
import static com.sise.common.utils.JWTUtils.getTokenInfo;
...
public class AccessFilter extends BaseFilter {
    ...
    public Object run() throws ZuulException {
        ...
        String uri = request.getRequestURI();  // /server/menu/admin/listUserMenus
        ...
        try {
            ...
            //拿到token和redis缓存中的token进行比较
            if (Objects.isNull(redisUtil.get(TOKEN_ALLOW_LIST + id))) {
                errorResponse(TOKEN_EXPIRED.getDesc(), TOKEN_EXPIRED.getCode(), 200);
            }

            // /api/....   截取第二个/以即其之后的字符串
            // /api/server/....   截取第三个/以即其之后的字符串
            String result = uri.substring(uri.indexOf("/server") + 7); // /menu/admin/listUserMenus

            String permission = method + result;    //自己定义的鉴权标识符 GET/menu/admin/listUserMenus
            ...
            List<String> userResource = (List<String>) redisUtil.get(USER_RESOURCE_KEY + String.valueOf(id));
